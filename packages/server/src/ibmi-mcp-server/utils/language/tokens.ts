      ...
      match: [
        {
          type: `word`,
          match: (value: string) => {
            return [`IN`, `OUT`, `INOUT`].includes(value.toUpperCase());
          },
        },
      ],
      becomes: `parmType`,
      ...
      match: [
        {
          type: `word`,
          match: (value: string) => {
            return value.toUpperCase() === `ORDER`;
          },
        },
        {
          type: `word`,
          match: (value: string) => {
            return value.toUpperCase() === `BY`;
          },
        },
      ],
      becomes: `clause`,
      ...
      match: [
        {
          type: `word`,
          match: (value: string) => {
            return [`INNER`, `EXCEPTION`, `CROSS`, `LEFT`, `RIGHT`].includes(
              value.toUpperCase(),
            );
          },
        },
      ...
      match: [
        {
          type: `word`,
          match: (value: string) => {
            return [`FULL`, `LEFT`, `RIGHT`].includes(value.toUpperCase());
          },
        },
        {
          type: `word`,
          match: (value: string) => {
            return value.toUpperCase() === `OUTER`;
          },
        },
        {
          type: `word`,
  ...
  tokenise(content: string) {
    ...
    for (let i = 0; i < content.length; i++) {
      ...
      if (
        ...
        content[i] &&
        content[i + 1] &&
        content.substring(i, i + 2) === this.startCommentString
      ) {
        commentStart = i;
        ...
        if (this.storeComments) {
          result.push({
            value: content.substring(commentStart, i),
            type: `comment`,
            range: { start: commentStart, end: i },
          });
        }

        content =
          content.substring(0, commentStart) +
          ` `.repeat(preNewLine - commentStart) +
          content.substring(preNewLine);
        ...
        content =
          content.substring(0, commentStart) +
          ` `.repeat(endOfBlock - commentStart) +
        ...
        state === ReadState.IN_STRING &&
        content[i] !== this.stringChar
      ) {
        currentText += content[i];

        // Handle when we're in a name
      } else if (
        state === ReadState.IN_NAME &&
        content[i] !== this.delimNameChar
      ) {
        currentText += content[i];
      } else {
        switch (content[i]) {
          ...
          default:
            if (
              this.splitParts.includes(content[i]) &&
              state === ReadState.NORMAL
            ) {
              if (currentText.trim() !== ``) {
                result.push({
              ...
              if (!this.spaces.includes(content[i])) {
                result.push({
                  value: content[i],
                  type: this.types[content[i]],
                  range: { start: i, end: i + content[i].length },
                });
              }
    ...
    for (let i = 0; i < tokens.length; i++) {
      for (let y = 0; y < SQLTokeniser.matchers.length; y++) {
        ...
        if (
          i > 0 &&
          i < tokens.length - 2 &&
          tokens[i].value.toLowerCase() === "for" &&
          tokens[i - 1].type === "closebracket" &&
          tokens[i + 2].value.toLowerCase() === "data"
        ) {
        ...
        if (goodMatch) {
          ...
          const value = state.content.substring(
            matchedTokens[0].range.start,
            matchedTokens[matchedTokens.length - 1].range.end,
          );
