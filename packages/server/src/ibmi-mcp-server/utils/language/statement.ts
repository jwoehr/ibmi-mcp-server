...
export default class Statement {
  ...
  getTokenByOffset(offset: number) {
    const blockSearch = (tokens: Token[]): Token | undefined => {
      const token = tokens.find(
        (token) => offset >= token.range.start && offset <= token.range.end,
      );
  ...
  getBlockRangeAt(offset: number): IRange | undefined {
    const blockContainsOffset = (
      cOffset: number,
      block: Token[],
    ): IRange | undefined => {
      const tokenInOffset = block.find(
        (token) => cOffset >= token.range.start && cOffset <= token.range.end,
      );
      if (tokenInOffset) {
        if (tokenInOffset.type === `block`) {
          if (tokenInOffset.block!.length > 0) {
              ...
              blockRange = {
                start:
                  this.tokens.findIndex(
                    (token) => token.range.start === tokenInOffset.range.start,
                  ) + 1,
                end: this.tokens.findIndex(
                  (token) => token.range.end === tokenInOffset.range.end,
                ),
              };
            }
            ...
            const rawEnd = this.tokens.findIndex(
              (token) => token.range.end === tokenInOffset.range.end,
            );
            return {
              start: rawEnd,
          ...
          const rawStart = this.tokens.findIndex(
            (token) => token.range.start === block[0].range.start,
          );
          const rawEnd = this.tokens.findIndex(
            (token) => token.range.end === block[block.length - 1].range.end,
          );
          return {
            start: rawStart,
            end: rawEnd + 1,
  ...
  getCallableDetail(offset: number, withBlocks = false): CallableReference {
    ...
    if (range) {
      const hasDot =
        (tokenIs(this.tokens[range.start - 4], `word`) ||
  ...
  getBlockAt(offset: number): Token[] {
    const expandBlock = (tokens: Token[]): Token[] => {
      ...
      if (block.length > 0) {
        return block.reduce(
          (acc, token) => acc.concat(expandBlock(token.block!)),
          tokens,
        );
      }
        ...
        if (tokenIs(statementBlock, `block`)) {
          cteList.push({
            ...
            statement: new Statement(
              Statement.trimTokens(statementBlock.block),
  ...
  getRoutineParameters(): ObjectRef[] {
    ...
    if (firstBlock && firstBlock.block) {
      ...
      for (const parameter of parameters) {
        ...
        const definitionTokens = (nameIndex === 1 ? [parameter[0]] : []).concat(
          parameter.slice(nameIndex + 1),
        );
  ...
  getObjectReferences(): ObjectRef[] {
    ...
    const basicQueryFinder = (startIndex: number): void => {
      ...
      for (let i = startIndex; i < this.tokens.length; i++) {
        if (tokenIs(this.tokens[i], `clause`, `FROM`)) {
          ...
          (currentClause === `from` && tokenIs(this.tokens[i], `clause`)) ||
        ...
        if (
          tokenIs(this.tokens[i], `clause`, `FROM`) ||
    ...
    switch (this.type) {
      ...
      case StatementType.Create:
        ...
        if (
          tokenIs(this.tokens[1], `keyword`, `OR`) &&
          ...
          switch (object.createType?.toUpperCase()) {
            ...
            case `PROCEDURE`:
              ...
              for (let i = postName; i < this.tokens.length; i++) {
                if (
                  tokenIs(this.tokens[i], `keyword`, `SPECIFIC`) &&
                  this.tokens[i + 1]
                ) {
                ...
                if (
                  tokenIs(this.tokens[i], `keyword`, `EXTERNAL`) &&
            ...
            case `VARIABLE`:
              if (postName) {
                object.createType = Statement.formatSimpleTokens(
                  this.tokens.slice(postName),
                );
              }
              break;
            ...
            case `VIEW`:
            case `TABLE`:
              const asKeyword = this.tokens.findIndex((token) =>
                tokenIs(token, `keyword`, `AS`),
              );
      ...
      case StatementType.Declare:
        if (tokenIs(this.tokens[2], `keyword`, `HANDLER`)) {
          ...
          (tokenIs(this.tokens[1], `word`, `GLOBAL`) &&
          ...
          if (tokenIs(this.tokens[2], `keyword`, `CURSOR`)) {
            ...
            let defaultIndex = this.tokens.findIndex((t) =>
              tokenIs(t, `keyword`, `DEFAULT`),
            );

            const valueTokens = this.tokens.slice(
              2,
  ...
  private getRefAtToken(
    i: number,
    options: { withSystemName?: boolean; includeParameters?: boolean } = {},
  ): ObjectRef | undefined {
    ...
    const isSubSelect =
      tokenIs(nextToken, `function`, `TABLE`) ||
        ...
        if (
          ...
          sqlObj = {
            ...
            object: {
              ...
              name:
                nextToken && NameTypes.includes(nextToken.type)
                  ? nextToken.value
                  : undefined,
            },
          };
    ...
    if (sqlObj) {
      if (options.withSystemName !== true) {
        ...
        if (nextToken && this.tokens[nextIndex + 1]) {
          if (
            tokenIs(this.tokens[nextIndex + 1], `keyword`, `AS`) &&
      ...
      if (!isSubSelect && !sqlObj.isUDTF) {
        ...
        if (
          ...
          const blockTokens = this.getBlockAt(
            this.tokens[endIndex + 1].range.end + 1,
          );
  ...
  getEmbeddedStatementAreas() {
    ...
    const validIntoStatements: StatementType[] = [
